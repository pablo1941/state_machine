...
/**
  * @}
  */
void escribirEEPROM(uint16_t direccion, uint8_t value)
{
	FLASH_Unlock(FLASH_MEMTYPE_DATA);

	 FLASH_EraseByte(direccion);	
   delayEE();
	//  delay_ms(40);	
	 FLASH_ProgramByte(direccion, value);	
   delayEE();
	//  delay_ms(40);	
...
{
	return FLASH_ReadByte(direccion);
}

void actualizarEEPROM(uint16_t direccion, uint8_t value)
{
  uint8_t valorActual = leerEEPROM(direccion);

  if(valorActual != value)
  {
    escribirEEPROM( direccion, value);
  }
}
